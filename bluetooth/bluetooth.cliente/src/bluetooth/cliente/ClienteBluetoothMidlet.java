...
public class ClienteBluetoothMidlet extends MIDlet implements RecepcaoDeDados, CommandListener {
        ...
        if(gui.actualID == gui.ECRA_MENU_PRINCIPAL) {
            ...
            if(command.getCommandType() == command.BACK) {
                ...
                if(cliente.estaAutenticado()) {
                    mostrarAlerta("hmm", "a sacar email...", AlertType.INFO, 0);
                    comunicacoes.buscarMail(rms.getUltimoAcesso());
                } else if(comunicacoes.getEstadoLigacao() != comunicacoes.ESTADO_LIGADO) {
                    mostrarAlerta("argh!!", "nao estas ligado a nenhum servidor...", AlertType.ERROR, 0);
                } else {
                    mostrarAlerta("oops", "tens que te autenticar!", AlertType.ERROR, 0);
                }
            } else {
            ...
            if(command.getCommandType() == command.BACK) {
                ...
                if(comunicacoes.getEstadoLigacao() == comunicacoes.ESTADO_DESLIGADO || comunicacoes.getEstadoLigacao() == comunicacoes.ESTADO_LOGGED_OUT) {
                    ...
                    new Thread() {
                        public void run() {
                            ...
                            if(comunicacoes.getEstadoLigacao() == comunicacoes.ESTADO_LIGADO) {
                                ...
                                mostrarAlerta("muito bem...", "estas ligado a um servidor\n\na fazer autenticacao.", AlertType.INFO, 0);
                                
                                setMenuRetorno(gui.ECRA_MENU_PRINCIPAL);
                            } else {
                                setEcra(gui.ECRA_AUTENTICACAO);
                                mostrarAlerta("erro...", "nao foi possivel estabelecer uma ligacao ao um servidor.", AlertType.INFO, 0);
                    ...
                    mostrarAlerta("erro...", "ja tens uma ligacao estabelecida. tens que fazer logout primeiro.", AlertType.INFO, 0);
                }
            }
            ...
            if(command.getCommandType() == command.BACK) {
                ...
                if (comunicacoes.getEstadoLigacao() != comunicacoes.ESTADO_LIGADO) {
                    mostrarAlerta("oops", "ainda nao estas ligado a um servidor", AlertType.ERROR, 0);
                } else if (!cliente.estaAutenticado()) {
                    mostrarAlerta("oops", "tens que te autenticar", AlertType.ERROR, 0);
                } else {
    ...
    public void guardarMail(int id) {
        ...
        if(rms.inserir(mail)) {
            ...
            mostrarAlerta("eeek!", "erro ao gravar o email", AlertType.ERROR, 0);
        }
    }
    ...
    public void executarAccaoProtocolo(int accao, String msg) {
        ...
        if(accao == Protocolo.INVALIDO) {
            ...
            setEcra(menuRetorno);
            mostrarAlerta("yay...", "foste autenticado com sucesso!", AlertType.INFO, 0);
            ...
            mostrarAlerta("oops...", "nome de utilizador ou password incorrecto!", AlertType.INFO, 0);
            ...
            mostrarAlerta("ehe", "tens " + (BaseDeDados.getTamanhoLista(lista) - numMailsAntes) + " mails novos.", AlertType.INFO, 0);
            preencherListaDeMails();
            
            
        } else if(accao == Protocolo.OK_MAIL_ENVIADO_SUCESSO) {
            setEcra(gui.ECRA_MENU_PRINCIPAL);
            mostrarAlerta("info", "o teu email foi enviado com sucesso!", AlertType.INFO, 0);
            ...
            setEcra(gui.ECRA_MENU_PRINCIPAL);
            mostrarAlerta("erro", "existe um problema qualquer com o servidor!", AlertType.INFO, 0);
            
            
        } else if(accao == Protocolo.ERRO_ENDERECO_MAIL_INVALIDO_OU_REJEITADO) {
            setEcra(gui.ECRA_ENVIAR_PARA);
            mostrarAlerta("erro", "o envio de mail falhou porque um ou mais enderecos de envio sao invalidos!", AlertType.INFO, 0);
            ...
            setEcra(gui.ECRA_LISTAGEM_MAILS);
            mostrarAlerta("oops", "nao tens mails novos", AlertType.ERROR, 0);
            preencherListaDeMails();
        }
    }
        ...
        new Thread() {
            public void run() {
                ...
                if(comunicacoes.getEstadoLigacao() != comunicacoes.ESTADO_LOGGED_OUT) {
                    mostrarAlerta("!!!", "a ligacao ao servidor foi perdida.\nvao ser feitas novas tentativas de ligacao.", AlertType.ERROR, 0);
